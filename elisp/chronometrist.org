...
**** record-iterator                                    :generic:function:
#+BEGIN_SRC emacs-lisp
...
**** loop-records                                                  :macro:
I wanted a generic =cl-loop= interface to iterate over records in any backend, and this is the best way I could find to make it work. The =save-excursion= restores point for text backends, and (hopefully) has no effect for other backends.
...
**** sexp-iterator-helper                                      :procedure:
#+BEGIN_SRC emacs-lisp
