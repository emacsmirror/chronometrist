...
2. Permit forward/backward scrolling through dates + input a specific date.
3. Reexamine how to toggle display of key-values and tags.
...
To disable display of tags, customize `chronometrist-details-table-format'.")
#+END_SRC
...
a format string consuming a single argument passed to `format', or
a function of one argument (the key-values as a plist),
which must return the string to be displayed.
...
#+BEGIN_SRC emacs-lisp
(defcustom chronometrist-details-table-format
  [("#" 3 t)
   ("Task" 20 t)
   ("Tags" 20 t)
...
(defun chronometrist-details-entries ()
  "Return entries to be displayed in the `chronometrist-details' buffer.
  ...
  (cl-loop with index = 1
    ...
    (-let* (((&plist :name name :tags tags :start start :stop stop) plist)
            (tags (if tags
                    ...
                    ""))
            (start         (chronometrist-iso-timestamp-to-ts start))
            (stop          (chronometrist-iso-timestamp-to-ts stop))
...
(define-derived-mode chronometrist-details-mode tabulated-list-mode "Details"
  ...
  (make-local-variable 'tabulated-list-format)
  (setq tabulated-list-format chronometrist-details-table-format)
  (make-local-variable 'tabulated-list-entries)
