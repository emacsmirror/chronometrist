...
**** loop-records                                                  :macro:
I wanted a generic =cl-loop= interface to iterate over records in any backend, and this is the best way I could find to make it work.
...
(with-current-buffer (chronometrist-backend-file chronometrist-plist-test-backend)
  (mapcar
   (lambda (plist)
