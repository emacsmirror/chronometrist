...
1. edebug =chronometrist-plist-pp-buffer= (and others if desired),
2. insert an s-expression in a new =emacs-lisp-mode= buffer, ensuring it does not contain newlines (other than in strings),
3. place point at the beginning of the buffer, and
4. =M-: (chronometrist-plist-pp-buffer) RET=.
...
**** tests
#+BEGIN_SRC emacs-lisp :tangle chronometrist-tests.el :load test
(ert-deftest chronometrist-plist-pp-buffer ()
  (should
   (equal
