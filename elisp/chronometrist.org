...
:PROPERTIES:
:CUSTOM_ID: frontend-details-buffer-name-base
:END:
#+BEGIN_SRC emacs-lisp
...
**** buffer-name                                                  :reader:
:PROPERTIES:
:CUSTOM_ID: frontend-details-buffer-name
:END:
#+BEGIN_SRC emacs-lisp
...
:PROPERTIES:
:CUSTOM_ID: frontend-details-display-tags
:END:
...
:PROPERTIES:
:CUSTOM_ID: frontend-details-display-key-values
:END:
If the value of this variable is a function and the string it returns contains a newline, the results may be undesirable...but hardly unrecoverable, so try it and see, if you wish.
