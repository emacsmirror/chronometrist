...
keyword-values (except :start and :stop)."
  (-let* (((&plist :stop stop-1)   plist-1)
          ...
          (plist-1-no-time (chronometrist-plist-remove plist-1 :start :stop))
          (plist-2-no-time (chronometrist-plist-remove plist-2 :start :stop)))
    (and (time-equal-p stop-1-unix start-2-unix)
         (equal plist-1-no-time plist-2-no-time))))
