...
* Code
** Common
*** history-limit                                         :custom:variable:
Currently only used by [[#unified-prompt][chronometrist-key-values-unified-prompt]].

There is a possibility that, for a task which the user has not clocked into for a long time (such that the last record for this task is more than $HISTORY number of records away), no key/value history may be generated.

An alternative could be to count only matching records, instead of all records, as contributing to the limit. This may lead to the optimization being defeated if the matching records are very far apart, albeit under the limit.
