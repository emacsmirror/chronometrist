...
(mapc #'chronometrist-reset-internal chronometrist-test-backends)
#+END_SRC
...
5. multiple records
6. [plist-group] latest plist is split
...
(ert-deftest chronometrist-backend-tests ()
  (unwind-protect
      (progn
        ;; no file
        (should-error (chronometrist-latest-date-records chronometrist-test-backend))
        (should-error (chronometrist-insert chronometrist-test-backend
                                (car chronometrist-test-records)))
        (should-error (chronometrist-remove-last chronometrist-test-backend))
        (should-error (chronometrist-to-list chronometrist-test-backend))

        (should-error (chronometrist-latest-record chronometrist-test-backend))
        (should-error (chronometrist-task-records-for-date chronometrist-test-backend
                                               "Test" (chronometrist-date-ts)))
        (should-error (chronometrist-active-days chronometrist-test-backend
                                     "Test"))
        (should-error (chronometrist-replace-last chronometrist-test-backend
                                      (cl-second chronometrist-test-records)))

        (should (chronometrist-create-file chronometrist-test-backend))
        ;; (should (chronometrist-replace-last chronometrist-test-backend ...))

        ;; (should (chronometrist-create-file chronometrist-test-backend))
        ;; (should (chronometrist-count-records chronometrist-test-backend))
        ;; (should (chronometrist-to-list chronometrist-test-backend))
        ;; (should (chronometrist-to-hash-table chronometrist-test-backend))
        ;; (should (chronometrist-to-file ... chronometrist-test-backend ...))
        ;; (should (chronometrist-backend-empty-p chronometrist-test-backend))
        )
    ;; cleanup - remove test backend files
    (delete-file (chronometrist-backend-file chronometrist-test-backend))))
#+END_SRC
