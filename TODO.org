...
+ +=:row-transformer= and =:schema-transformer= functions+ a function to emit a single cell of your column; it has access to a single row of table data
+ (optional) =:setup= and =:teardown= forms.
