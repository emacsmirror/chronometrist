...
3. A custom variable containing a list of tasks
   #+BEGIN_SRC emacs-lisp
   ("A Task Name"
...
4. Completion for sub-plists - if the value of a user keyword-value pair is a plist (heuristic - list with keyword as first element), can we reuse the keyword-value prompt for it? 🤔
...
7. Display task's weekly progress using ASCII/sparklines
